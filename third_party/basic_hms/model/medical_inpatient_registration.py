...
#     medical_hospital_bed_id = fields.Many2one('medical.hospital.bed',string="Hospital Bed",required=True)
    ...
    admission_type = fields.Selection([('routine','Rutinario'),('maternity','Maternidad'),('elective','Electivo'),('urgent','Urgente'),('emergency','Emergencia  ')],required=True,string="Tipo de admision")
    medical_pathology_id = fields.Many2one('medical.pathology',string="Razon para la admicion")
    info = fields.Text(string="Extra Info")
    bed_transfers_ids = fields.One2many('bed.transfer','inpatient_id',string='Transferencia de cama',readonly=True)
    ...
    state = fields.Selection([('free','Gratuito'),('confirmed','Confirmado'),('hospitalized','Hospitalizado'),('cancel','Anulado'),('done','Alta')],string="Estado",default="free")
